  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        const finalImageUrl = baseUrl + res.data.imageUrl;
            ...
            <div className="w-full md:w-1/3 flex flex-col items-center space-y-4">
              <div className="relative group">
                <div className="w-48 h-48 rounded-3xl bg-slate-100 border-4 border-white shadow-xl overflow-hidden flex items-center justify-center relative">
                  {formData.image ? (
                    <img src={formData.image.startsWith('http') ? formData.image : `${import.meta.env.VITE_API_URL.replace('/api', '')}${formData.image}`} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
